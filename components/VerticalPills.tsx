...
// # of full cycles before wrapping around
const SCROLL_REPEAT_FACTOR = 5;
...
const getCircleBackgroundColor = (categoryName: string): string => {
  const colorMap: Record<string, string> = {
    Frameworks: "rgba(127, 255, 212, 0.8)",
    Databases: "rgba(240, 230, 140, 0.8)",
    Tools: "rgba(147, 112, 219, 0.8)",
    Others: "rgba(221, 160, 221, 0.8)"
  };
  ...
  if (imageError) {
    return (
      ...
      >
        ...
        {showTooltip && (
          <div className="hidden md:block fixed left-1/2 transform -translate-x-1/2 mb-2 px-4 py-3 bg-black/90 text-white text-sm font-medium rounded-lg whitespace-nowrap z-[9999] pointer-events-none shadow-lg border border-white/20">
            {displayName}
          </div>
        )}
      </div>
  ...
  return (
    <div
      className={`relative ${className} ${
        websiteUrl ? "cursor-pointer" : "cursor-default"
      }`}
      onMouseEnter={() => setShowTooltip(true)}
      onMouseLeave={() => setShowTooltip(false)}
      onClick={handleClick}
    >
      ...
      {/* Enhanced Tooltip with click instruction */}
      {showTooltip && (
        <div className="hidden md:block fixed top-[5%] left-1/2 transform -translate-x-1/2 mb-2 px-2 py-2 bg-black/90 text-white text-wrap text-xs font-small rounded-lg whitespace-nowrap z-[9999] pointer-events-none shadow-2xl border border-white/20">
          {displayName}
        </div>
      )}
    </div>
...
interface SinglePillProps {
  ...
  index: number;
  globalStep: number;          // parent "metronome"
...
function SinglePill({
  ...
  // 🔹 local step for this pill (position)
  ...
  const extendedItems = Array(SCROLL_REPEAT_FACTOR + 2)
  .fill(null)
  .flatMap(() => category.items);

  const maxVisibleItems = mounted && isMobile ? 3 : 5;
  const visibleItems: { item: string; key: string; index: number }[] = [];

  if (isExpanded) {
    for (
      let i = 0;
      i < Math.min(maxVisibleItems, category.items.length);
      i++
    ) {
  ...
  const isCarouselPhase = hasJoinedGlobal && isGlobalPlaying;

  // 🔹 per-p
  const itemHeight = isMobile ? 72 : 92;
  const totalHeight = category.items.length * itemHeight * SCROLL_REPEAT_FACTOR;
  const scrollY =
    totalHeight === 0
      ? 0
      : -((localStep * itemHeight) % totalHeight);

  const handlePillMouseEnter = () => {
    if (isCarouselPhase) setIsPaused(true);
  };

  const handlePillMouseLeave = () => {
    if (isCarouselPhase) setIsPaused(false);
  };

  // Increment localStep on each globalStep tick (if not paused)
  useEffect(() => {
    if (!isCarouselPhase) return;
    if (isPaused) return;
    setLocalStep(prev => prev + 1);
  }, [globalStep, isCarouselPhase, isPaused]);
          ...
          <div className="absolute inset-3 md:inset-6 flex items-start justify-center">
            {isExpanded && (
              <div
                ...
                style={{
                  height: mounted && isMobile ? "210px" : "450px"
                }}
              >
                ...
                {!isCarouselPhase && (
                  <AnimatePresence mode="popLayout" key={animationKey}>
                    <motion.div
                      ...
                      exit="exit"
                      onAnimationComplete={() => {
                        if (hasJoinedGlobal) return;
                        setTimeout(() => {
                          setHasJoinedGlobal(true);
                          onReadyForCarousel();
                        }, timing.carouselStartDelay);
                      }}
                    >
                      {visibleItems.map(
                        ({ item, key, index: itemIndex }) => (
                          <motion.div
                            key={key}
                ...
                {/* Carousel continuous rolling phase */}
                {isCarouselPhase && (
                  <motion.div
                  ...
                  >
                    {extendedItems.map((item, idx) => (
                      <div
                        ...
                        style={{
                          backgroundColor:
                            getCircleBackgroundColor(category.name)
                        }}
                      >
                        <TechIcon
                          techName={item}
...
export default function VerticalPills() {
  ...
  const [readyPills, setReadyPills] = useState(0);

  const isGlobalPlaying = readyPills > 0;

  const handlePillReadyForCarousel = useCallback(() => {
    setReadyPills((count) => count + 1);
  }, []);

  useEffect(() => {
    if (!isGlobalPlaying) return;
    const interval = setInterval(() => {
      setGlobalStep((step) => step + 1);
    }, 1500);
    return () => clearInterval(interval);
  }, [isGlobalPlaying]);

  return (
    <div className="relative w-full py-10 md:py-8 sm:py-12">
      <div className="w-full md:w-[46%] max-w-7xl mx-auto px-4">
        <div
          className={clsx(
            "place-items-center",
            mounted
              ? "grid grid-cols-4 gap-1 md:flex md:gap-8 lg:gap-20 md:items-center md:justify-center"
          ...
          {PILL_CATEGORIES.map((category, index) => (
            ...
            >
              <SinglePill
                ...
                onReadyForCarousel={handlePillReadyForCarousel}
              />
            </motion.div>
          ))}
        </div>
      ...
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-gradient-radial from-white/3 to-transparent rounded-full blur-3xl" />
      </div>
    </div>
  );
}
